...
main(void)
{
    ...
    if (k_ArenaInit(&arena, K_SIZE_1M * 60, k_getPageSize()))
    {
        ...
        ArenaNodeI64 anNumber = {0};

        K_ARENA_SCOPE(&arena)
        {
            k_ArenaNodeAlloc(&arena, (k_ArenaNodeAllocOpts){
                .pNode = &anNumber.node,
            ...
            *anNumber.pI = 666;
            ...
            k_print(&k_GpaInst()->base, stdout, "pos in scope: {ssize_t}, number: {:#x:u64}, p777: {i64}\n", arena.pos, anNumber.pI, *p777);
        }

        k_print(&k_GpaInst()->base, stdout, "pos after: {sz}, number: '{:#x:u64}'\n", arena.pos, (uint64_t)anNumber.pI);
    }
